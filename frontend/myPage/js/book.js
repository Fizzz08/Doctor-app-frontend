...
function displayTimeSlots(selectedDate, doctorId) {
    ...
    const availableSlotsFetch = fetch(`http://localhost:8080/api/v1/doctor/availableSlot?doctorId=${doctorId}`, {
        method: 'GET',
        headers: {
    ...
    const bookedSlotsFetch = fetch(`http://localhost:8080/api/v1/doctor/bookedSlots?doctorId=${doctorId}&date=${formattedDate}`, {
        method: 'GET',
        headers: {
            'Authorization': 'Bearer ' + tkn
        }
...
async function confirmBooking() {
    try {
        ...
        const doctorResponse = await fetch(`http://localhost:8080/api/v1/doctor/details/${doctor.doctorId}`, {
            headers: {
                'Authorization': 'Bearer ' + tkn
            }
        });
